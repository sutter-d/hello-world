...
def main():
    ...
    mpn = mpn[['ind', 'key']].rename(
        columns={'ind': 'cpn', 'key': 'mpn'}).drop_duplicates()
    ...
    logging.info("START PULLING CPN TO MPN KEY FROM GDRIVE")
    ...
    logging.info("FINISH PULLING CPN TO MPN KEY FROM GDRIVE")
    ...
    comps_forecast = comps_forecast.loc[(
    ...
    comps_forecast = comps_forecast.loc[(
        ~comps_forecast['cpn'].str.startswith("999"))]
    ...
    comps_forecast_mpn_proc['mp_ok'] = comps_forecast_mpn_proc.apply(lambda x: colcomp(
        x['evt1']+x['evt2']+x['dvt']+x['pvt']+x['mp'], x['total_qty']), axis=1)
    ...
    csv = reports + 'Component_Forecast_Analysis_' + \
        time.strftime("%Y-%m-%d-%H%M%S") + '.xlsx'
