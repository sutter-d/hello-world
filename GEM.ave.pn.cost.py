...
# In[]
avewwt = ogcaaswwt[ogcaaswwt['Unit Status'] != 'Cancelled']
avewwt = avewwt[avewwt['CapExp'] != 'EXP']
avewwt['UnitPrice'] = avewwt['PO Total Price (USD)'] / avewwt['Qty']
avewwt = avewwt[['Plant', 'P&G Part #', 'Supplier', 'UnitPrice']]
avewwt = avewwt.groupby(['P&G Part #', 'Supplier']).agg('mean').reset_index()

avevz = ogcaasvz[ogcaasvz['Unit Status'] != 'Cancelled']
avevz = avevz[avevz['Supplier'] != 'World Wide Technology']
avevz = avevz[avevz['CapExp'] != 'EXP']
avevz['UnitPrice'] = avevz['PO Total Price (USD)'] / avevz['Qty']
avevz = avevz[['Plant', 'P&G Part #', 'Supplier', 'UnitPrice']]
avevz = avevz.groupby(['P&G Part #', 'Supplier']).agg('mean').reset_index()

avecaas = avewwt.append(avevz)
#avecaas = avecaas.pivot(columns='Supplier', values='PO Total Price (USD)').reset_index('P&G Part #')
avecaas = pd.pivot_table(avecaas,
                         index=['P&G Part #'],
                         columns='Supplier',
                         values='UnitPrice').reset_index('P&G Part #')

csv = edr + "CaaSPartPrice.update." + time.strftime("%Y%m%d-%H%M%S") + ".csv"
avecaas.to_csv(csv, index=False)
...
#
#list_of_vz = ['Verizon', 'VERIZON', 'Westcon', 'NCR']
#
...
#
#cbtsforecast = forecast[forecast['Supplier'] == 'CINCINNATI BELL TECHNOLOGY SOLUTIONS INC'].copy()
